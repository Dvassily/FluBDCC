...
public class Map {
	 ...
	 *            vertical dimensions
	 */
	public Map(int x, int y, NeighbourhoodStrategy ns) {
	...
	public void moveRandomly(int x, int y) {
		...
		while ((!moveSucceded) && (!list.isEmpty())) {
			switch (list.pop()) {
			case NORTH:
				moveSucceded = moveToNorth(x, y);
				break;
			case EAST:
				moveSucceded = moveToEast(x, y);
				break;
			case SOUTH:
				moveSucceded = moveToSouth(x, y);
				break;
			case WEST:
				moveSucceded = moveToWest(x, y);
				break;
			}
		}
		System.out.println(" ");
	...
	/**
	 * Returns the neighbourhood of the case of coordinates (x,y) This is to say
	 * the entities which are on the 4x4 square around the case (x, y)
	 */
